...
class LiberoRunner():
    ...
    def run_policy_in_env(self, env_name, policy, render=False,prompt=None):
        ...
        # env._env.close()
        # Can this make any difference? By closing the environment
        env.close()
        gc.collect()
        del env
        # TODO: envs are not being closed properly hence getting EGL error 
    
    def run_episode(self, env, env_name, policy, init_states_, env_num, render=False,prompt=None):
        ...
        # 可以在这里修改task embedding了，可以根据observation修改task embedding
        task_emb = self.benchmark.get_task_emb(task_id).repeat(env_num, 1)
        if prompt is not None:
            task_emb = lu.get_task_embs("clip",prompt).repeat(env_num, 1)
        steps = 0

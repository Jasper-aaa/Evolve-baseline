...
set_libero_default_path("/home/yeyifan/workplace/embodied/Evolve-baseline/LIBERO/libero/libero")
...
def get_task_embs(task_embedding_format, descriptions):
    ...
    if task_embedding_format == "bert":
        ...
        # offline 
        tz = AutoTokenizer.from_pretrained("/home/public_data/libero/pretrained_model/clip/", clean_up_tokenization_spaces=True)
        model = AutoModel.from_pretrained("/home/public_data/libero/pretrained_model/clip/")
        tokens = tz(
            text=descriptions,  # the sentence to be encoded
            add_special_tokens=True,  # Add [CLS] and [SEP]
            max_length=25,  # maximum length of a sentence
